...
// Uses Intl with the en-CA locale, which natively formats as YYYY-MM-DD,
// so no manual offset arithmetic is needed and the result tracks the
// real timezone (correct even if Costa Rica ever adopts DST).
...
// True iff the given YYYY-MM-DD date falls on Saturday or Sunday.
// Anchored at noon to dodge any DST/midnight edge cases.
...
/**
 * Returns the next `n` weekday date strings (YYYY-MM-DD) starting from today
 * in Costa Rica time. Saturdays and Sundays are skipped because the cafeteria
 * does not operate on weekends (P1-3).
 */
export function getUpcomingDayKeys(n = 7) {
  const days = [];
  const now  = Date.now();
  for (let i = 0; days.length < n && i < n * 3; i++) {
    const key = getDayKey(new Date(now + i * 24 * 60 * 60 * 1000));
...
function normalizePaymentStatus(status) {
  // Use the canonical payment-status mapper so dashboard, deliveries, and
  // admin-orders all surface the same string ("CONFIRMADO" / "PENDIENTE_DE_PAGO").
  // Previously this returned "PAGADO" which drifted from api/deliveries.js (P3-1).
  const s = String(status || "").toUpperCase();
  if (s === "PAGADO" || s === "CONFIRMADO" || s === "CONFIRMADO_SINPE") return "CONFIRMADO";
...
export function buildDashboardSnapshot(settings, menu, orders, stats = null) {
  ...
  return {
    ...
    // No fallback menu: returning null forces the UI to render an empty-state
    // panel and disable the submit button instead of charging the customer
    // ₡1,000 for a placeholder "Menu no configurado".
