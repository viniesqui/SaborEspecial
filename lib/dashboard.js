...
function getPaymentStatusLabel(value) {
  ...
  if (normalized === "PAGADO" || normalized === "PENDIENTE_DE_PAGO") {
    return normalized.replaceAll("_", " ");
  }
  if (normalized === "POR_VERIFICAR") {
    return "PENDIENTE DE PAGO";
  }
  if (normalized === "PENDIENTE_ENTREGA") {
    return "PAGADO";
  }
...
export function buildDashboardSnapshot(settings, menu, orders) {
  ...
  return {
    ...
    orders: orders.map((item) => ({
      ...
      paymentConfirmedAtLabel: item.paymentConfirmedAt ? formatTimestamp(item.paymentConfirmedAt) : "",
      deliveredAtLabel: item.deliveredAt ? formatTimestamp(item.deliveredAt) : ""
    }))
  };
}
